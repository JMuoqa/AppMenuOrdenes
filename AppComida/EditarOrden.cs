...
namespace AppComida
{
    public partial class EditarOrden : Form
    {
        ...
        private void entrada_busqueda_ordenes_TextChanged(object sender, EventArgs e)
        {
            ...
            else if (entrada_busqueda_ordenes.Text.StartsWith("-"))
                entrada_busqueda_ordenes.ForeColor = Color.Green;
            else if (entrada_busqueda_ordenes.Text == "#7/Julian/-3517787224")
                entrada_busqueda_ordenes.ForeColor = colorPlaceHolder;
            else
                entrada_busqueda_ordenes.ForeColor = Color.Black;
        }
        ...
        private void TodasLasEntradasNormales_Enter(object sender, EventArgs e)
        {
            ...
            if (caja.ForeColor == colorPlaceHolder)
            {
                caja.Text = "";
        ...
        private void TodasLasEntradasNormales_Leave(object sender, EventArgs e)
        {
            ...
            if (caja.ForeColor == Color.Black && (string.IsNullOrEmpty(caja.Text) || caja.Text == "$"))
            {
                ...
                if (caja.Name == "entrada_busqueda_ordenes")
                    caja.Text = "#7/Julian/-3517787224";
                else if (caja.Name == "entrada_busqueda_menu")
                    ...
                    caja.Text = "Busca un orden para editarla";
            }
        }
        #endregion
        ...
        {
            if (entrada_busqueda_ordenes.Text.StartsWith("#"))
            {
                BusquedaPorID_ParaOrden();
            }
            else if (entrada_busqueda_ordenes.Text.StartsWith("-"))
            {
                BusquedaPorTelefono_ParaOrden();
            }
            else
            {
                BusquedaPorNombre_ParaOrden();
            }
        }
        private void boton_busqueda_menu_Click(object sender, EventArgs e)
        {
            BusquedaPorNombre_ParaMenu();
        }
        private void entrada_busqueda_ordenes_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void BusquedaPorID_ParaOrden()
        {
            try
            {
                D_ConOrdenes conOrdenes = new D_ConOrdenes();
                if (string.IsNullOrWhiteSpace(entrada_busqueda_ordenes.Text) || entrada_busqueda_ordenes.Text.Length == 1)
                    throw new Exception("La entrada de busqueda esta vacia");
                int ID = int.Parse(entrada_busqueda_ordenes.Text.Replace("#", ""));
                var res = conOrdenes.ObtenerOrdenPorID(ID);
                if (!res.estado)
                    throw new Exception(res.mensaje);
        ...
        private void BusquedaPorNombre_ParaOrden()
        {
            try
            {
                D_ConOrdenes conOrdenes = new D_ConOrdenes();
                if (string.IsNullOrWhiteSpace(entrada_busqueda_ordenes.Text))
                    throw new Exception("La entrada de busqueda esta vacia");
                var res = conOrdenes.ObtenerOrdenPorNombre(entrada_busqueda_ordenes.Text);
                if (!res.estado)
                    throw new Exception(res.mensaje);
                DataTable dt = res.datos;
                if (dt.Rows.Count < 0)
        ...
        private void BusquedaPorTelefono_ParaOrden()
        {
            try
            {
                D_ConOrdenes conOrdenes = new D_ConOrdenes();
                if (string.IsNullOrWhiteSpace(entrada_busqueda_ordenes.Text))
                    throw new Exception("La entrada de busqueda esta vacia");
                string tel = entrada_busqueda_ordenes.Text.Replace("-", "");
                var res = conOrdenes.ObtenerOrdenPorTelefono(tel);
                if (!res.estado)
                    throw new Exception(res.mensaje);
                DataTable dt = res.datos;
                if (dt.Rows.Count < 0)
        ...
        private void BusquedaPorNombre_ParaMenu()
        {
            try
            {
                D_ConMenu conMenu = new D_ConMenu();
                if (string.IsNullOrWhiteSpace(entrada_busqueda_menu.Text))
                    throw new Exception("La entrada de busqueda esta vacia");
                var res = conMenu.ObtenerMenusPorNombre(entrada_busqueda_menu.Text);
                if (!res.estado)
                    throw new Exception(res.mensaje);
                DataTable dt = res.datos;
                if (dt.Rows.Count < 0)
        ...
        private void LlenarDatos(DataGridViewRow fila)
        {
            ...
            {
                D_ConOrdenes conOrdenes = new D_ConOrdenes();
                var res = conOrdenes.ObtenerDetalles(int.Parse(fila.Cells[0].Value.ToString()));
                if (!res.estado)
                    throw new Exception(res.mensaje);
                DataTable dt = res.datos;
                entrada_nombre_cliente.Text = fila.Cells[1].Value.ToString();
                entrada_nombre_cliente.ForeColor = Color.Black;
                entrada_numero.Text = fila.Cells[2].Value.ToString();
                ...
                entrada_hora_pedida.Text = horarioSeparado_E[0];
                entrada_hora_pedida.ForeColor = Color.Black;
                entrada_minuto_pedido.Text = horarioSeparado_E[1];
                entrada_minuto_pedido.ForeColor = Color.Black;
                entrada_hora_entrega.Text = horarioSeparado_E[0];
                entrada_hora_entrega.ForeColor = Color.Black;
        ...
        private void LLenarContenedorDePedidos(DataTable dt)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                string id = dt.Rows[i]["IDMenu"].ToString();
                if (panel_pedidos.Controls[$"contenedor_{id}"] != null)
                    throw new Exception("Ese menu ya esta agregado");
                string nombre = dt.Rows[i]["NombreMenu"].ToString();
                string cantidad = dt.Rows[i]["Cantidad"].ToString();
                string precioTotal = dt.Rows[i]["Precio"].ToString().Replace("$", "");
                int precioUnidad = int.Parse(precioTotal) / int.Parse(cantidad);
                //Aca le mando los argumentos para que se les asignen un name diferente
                LabelPerPedidos etiqueta_nueva_nombre = new LabelPerPedidos(id, "LPNombre");
                LabelPerPedidos etiqueta_nueva_precio = new LabelPerPedidos(id, "LPPrecioUni");
                LabelPerPedidos etiqueta_nueva_precioTotal = new LabelPerPedidos(id, "LPPrecioTotal");
                ...
                etiqueta_nueva_nombre.Text = nombre;
                etiqueta_nueva_precio.Text = $"Precio de unidad: ${precioUnidad}";
                etiqueta_cantidad.Text = $"Cantidad: {cantidad}";
                etiqueta_nueva_precioTotal.Text = $"Precio total: ${precioTotal}";
                Button btn_agregar = new Button();
                btn_agregar.BackColor = Color.Green;
                btn_agregar.Cursor = Cursors.Hand;
                btn_agregar.FlatAppearance.BorderSize = 0;
